= IN DEVELOPMENT, DON'T USE YET

== Memcached Manager

A sinatra memcached-manager that allows you to view status, flush/view keys and so on. Also easily pluggable to a Rails app.

{<img src="https://travis-ci.org/thiagofm/memcached-manager.png" />}[https://travis-ci.org/thiagofm/memcached-manager] {<img src="https://codeclimate.com/badge.png" />}[https://codeclimate.com/github/thiagofm/memcached-manager]

== Installation

1. Install the gem.

    gem install memcached-manager
    
2. Run memcached-manager

    memcached-manager

== Plugging memcached manager to an existing Rails app.

Todo.

== Contributing to Memcached Manager
 
* Check out the latest master to make sure the feature hasn't been implemented or the bug hasn't been fixed yet.
* Check out the issue tracker to make sure someone already hasn't requested it and/or contributed it.
* Fork the project.
* Start a feature/bugfix branch.
* Commit and push until you are happy with your contribution.
* Make sure to add tests for it. This is important so I don't break it in a future version unintentionally.
* Please try not to mess with the Rakefile, version, or history. If you want to have your own version, or is otherwise necessary, that is fine, but please isolate to its own commit so I can cherry-pick around it.

== Todo
* Think of a way to set up the Memcached variable dinamically(?)

== Copyright

Copyright (c) 2013 Thiago Fernandes Massa. See LICENSE.txt for
further details.

